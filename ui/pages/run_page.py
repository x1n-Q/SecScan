"""Run scan page – dark-themed progress, live logs, stop button."""
...
class RunPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._log_mode_combo.setMinimumWidth(200)
        self._log_mode_combo.setMinimumHeight(30)
        self._log_mode_combo.setStyleSheet(T.COMBO_STYLE)
        ...
        self._stop_btn.setStyleSheet(T.btn_style(T.DANGER, T.DANGER_HOVER))
        self._stop_btn.clicked.connect(self._on_stop)
        btn_row.addWidget(self._stop_btn)

        self._results_btn = QPushButton("View Results  →")
        self._results_btn.setMinimumHeight(38)
        self._results_btn.setEnabled(False)
        self._results_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._results_btn.setStyleSheet(T.btn_style(T.SUCCESS_HOVER, T.SUCCESS))
    ...
    def start_scan(
        self, tools: List[ToolBase], project_info: ProjectInfo, output_dir: str,
    ):
    ...
    def _on_finished(self, result: ScanResult):
        ...
        self._status_lbl.setText(
            f"Scan complete — {total} finding(s), {errors} error(s)"
        )
        self._stop_btn.setEnabled(False)
